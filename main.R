...
produceStr = function(solname){
    ...
    # print(solname["unit"])
    # print(whichMeasure(solname["unit"]))
...
# if (solname["unit"]=="W"){
#     kWhMonth = sumMonth(datTemp, "solD40JA_PV_W")
#     sumDay = integrateInterval(date(min(datTemp$timestamp)), days(1), datTemp, solName)
#     sumWeek = integrateInterval(date(min(datTemp$timestamp)), days(1), datTemp, solName)
#     print(kWhMonth)
# }
...
# sumDays %>% dplyr::filter(row_number() %% 3 != 1) ## Delete every 3rd row starting from 1
# yay = sumDays %>% dplyr::filter(row_number() %% 2 != 1) ## Delete every 2rd row starting from 1

print(head(sumDays))

ind <- seq(3, ncol(sumDays), by=2) # indices of columns to remove
sumDays = sumDays[, -ind]
sumWeeks = sumWeeks[, -ind]
print(head(sumDays))
print(sumWeeks)

# p = ggplot(sumDays, aes(x=day, y=D40JA)) + geom_point(shape = 0) +
# p = p + theme_minimal() + xlab("Day") + ylab("kWh") + ggtitle("Power generated by each solar panel")
# ggsave("weekKWH.pdf", width = width, height = height, units = "cm")
...
p = ggplot(sumDays, aes(x=day, y=value, color = variable)) +
    ...
    geom_point(aes(y = R13LG, col = "R13JA")) +
    theme_minimal() + xlab("Day") + ylab("kWh") + ggtitle("Power generated by each solar panel")
